#!/bin/bash
# Ralph v2 - AI agent loop with ravagents support
#
# Usage:
...
#
# When the PRD JSON contains "useRavagents: true", the loop drives both user story
# implementation tasks and the associated ravagent review tasks in sequence.
...
# Detect ravagents mode from PRD JSON
USE_RAVAGENTS="false"
if command -v jq &>/dev/null; then
  USE_RAVAGENTS=$(jq -r '.useRavagents // false' "$PRD_FILE" 2>/dev/null || echo "false")
else
  if grep -q '"useRavagents": *true' "$PRD_FILE" 2>/dev/null; then
    USE_RAVAGENTS="true"
  fi
fi
...
echo "  Ravagents mode: $USE_RAVAGENTS"
...
echo ""
echo "Ralph v2 reached max iterations ($MAX_ITERATIONS) without completing all tasks."
